...
export default function UsersScreen() {
  ...
  const [rows, setRows] = React.useState<
    { id: string; uid: string; displayName: string; inHome: boolean; role: string }[]
  >([]);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      setRows(users.map((u) => ({ id: u.id, uid: u.uid, displayName: u.displayName, inHome: u.inHome, role: u.role })));
    })();
    return () => {
      cancelled = true;
    };
